      ...
      private void performSchemaMigration() {
          try {
              ...
              if (e.getMessage().contains("no such column: lastSeen")) {
                  try {
                      Logger.info("Performing database schema upgrade: Adding 'lastSeen' column.");
                      playerStatsDao.executeRaw("ALTER TABLE player_stats ADD COLUMN lastSeen DATETIME;");
                      Logger.info("Database schema upgrade successful.");
                  } catch (SQLException ex) {
                      Logger.severe("Critical error during database schema upgrade.");
                      ex.printStackTrace();
                  }
              }
          }
